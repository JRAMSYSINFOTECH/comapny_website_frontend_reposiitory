    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      {[...Array(40)].map((_, i) => (
        <motion.div
          ...
          transition={{ duration: Math.random() * 10 + 15, repeat: Infinity, ease: "easeInOut", delay: Math.random() * 5 }}
        >
          <div className={`w-1 h-1 rounded-full ${i % 3 === 0 ? "bg-emerald-400" : i % 3 === 1 ? "bg-sky-400" : "bg-fuchsia-400"}`} />
        </motion.div>
      ))}
      {[...Array(12)].map((_, i) => (
        <motion.div
          ...
          style={{ width: "200px", top: `${Math.random() * 100}%`, left: `${Math.random() * 100}%` }}
          animate={{ x: ["-100%", "200%"], opacity: [0, 1, 0] }}
          transition={{ duration: Math.random() * 5 + 8, repeat: Infinity, ease: "linear", delay: Math.random() * 3 }}
        />
      ))}
      {[...Array(8)].map((_, i) => (
        <motion.div
          ...
          style={{ width: `${Math.random() * 100 + 50}px`, height: `${Math.random() * 100 + 50}px`, left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%` }}
          animate={{ scale: [1, 1.5, 1], opacity: [0.1, 0.3, 0.1], rotate: [0, 180, 360] }}
          transition={{ duration: Math.random() * 10 + 10, repeat: Infinity, ease: "easeInOut", delay: Math.random() * 2 }}
        />
      ))}
      {[...Array(6)].map((_, i) => (
        <motion.div
          ...
          transition={{ duration: Math.random() * 3 + 4, repeat: Infinity, ease: "linear", delay: Math.random() * 2 }}
        />
      ))}
      <motion.div
      ...
      {[...Array(15)].map((_, i) => (
        <motion.div
          ...
          transition={{ duration: Math.random() * 3 + 3, repeat: Infinity, ease: "easeInOut", delay: Math.random() * 3 }}
        />
      ))}
    </div>
  );
      ...
      >
        ...
        {[
          ...
          <motion.div
            ...
            animate={{ opacity: [0, 1, 1, 0], scale: [1.2, 1, 1, 1.2] }}
            transition={{ duration: 2.5, repeat: Infinity, ease: "easeInOut", repeatDelay: 1.5, delay: idx * 0.1 }}
          />
        ))}
      </motion.div>
      {[
        { pos: "-top-6 -left-6", gradient: "from-emerald-500 to-teal-500", icon: "⚡", delay: 0 },
        { pos: "-top-6 -right-6", gradient: "from-sky-500 to-blue-500", icon: "🔗", delay: 0.5 },
        { pos: "-bottom-6 -left-6", gradient: "from-fuchsia-500 to-pink-500", icon: "📊", delay: 1 },
        { pos: "-bottom-6 -right-6", gradient: "from-violet-500 to-purple-500", icon: "🔒", delay: 1.5 },
      ].map((item, idx) => (
        <motion.div
          key={idx}
          className={`absolute ${item.pos} w-12 h-12 bg-gradient-to-br ${item.gradient} rounded-2xl flex items-center justify-center text-2xl shadow-xl`}
          animate={{ y: [0, -10, 0], rotate: [0, idx % 2 === 0 ? 5 : -5, 0] }}
          transition={{ duration: 3 + idx * 0.3, repeat: Infinity, ease: "easeInOut", delay: item.delay }}
        >
          {item.icon}
        </motion.div>
      ))}
...
const stages = [
  { title: "QR Code Creation & Customization", desc: "Generate and manage unique QR codes for assets, people, documents, events, or locations, with customization suited to real-world use.", icon: "🎨" },
  { title: "Deployment & Distribution", desc: "Deploy QR codes across physical and digital touchpoints such as packaging, ID cards, signage, tickets, bins, and more.", icon: "🚀" },
  { title: "Engagement & Integration", desc: "When scanned, QR codes trigger defined actions such as opening content, capturing inputs, recording attendance, or interacting with connected systems.", icon: "🔗" },
  { title: "Monitoring & Analytics", desc: "Track scan activity, locations, devices, and usage trends to understand engagement and improve effectiveness.", icon: "📊" },
  { title: "Security & Access Control", desc: "Apply expiry rules, access permissions, and basic authentication to maintain control and reduce misuse.", icon: "🔒" },
];

const exampleApps = [
  { title: "Education", desc: "QR-based access for attendance tracking, homework distribution, and result viewing via student IDs or printed materials.", icon: "🎓" },
  { title: "Healthcare", desc: "QR-enabled check-ins, session identification, and controlled access to patient-related workflows.", icon: "🏥" },
  { title: "Smart Cities", desc: "QRs for asset identification, bin tracking, citizen feedback, and linking physical infrastructure to digital systems.", icon: "🏙️" },
  { title: "Museums & Temples", desc: "QR-powered audio guides, donations, ticketing, and multilingual content delivery.", icon: "🏛️" },
  { title: "Retail", desc: "Product information, authenticity checks, and loyalty engagement through on-product QR codes.", icon: "🛍️" },
  { title: "Events & Hotels", desc: "QR-based check-ins, booking confirmations, and access to digital services during events or stays.", icon: "🎫" },
];

const features = [
  { title: "Bulk & Custom QR Generation", desc: "Create and manage QR codes centrally using templates, customization options, and dynamic data — designed for operational use, not one-off campaigns.", icon: "📱", color: "from-emerald-500 to-teal-500" },
  { title: "Real-time Scan Insights", desc: "Monitor scans, locations, devices, and usage trends to understand engagement and improve outcomes.", icon: "📊", color: "from-sky-500 to-blue-500" },
  { title: "System Integrations", desc: "Connect QR codes with existing systems such as IoT devices, CRMs, payment flows, and messaging platforms.", icon: "🔗", color: "from-fuchsia-500 to-pink-500" },
  { title: "Security & Access Control", desc: "Support for expiry rules, permissions, authentication, and activity tracking to reduce misuse and improve governance.", icon: "🔒", color: "from-violet-500 to-purple-500" },
];
...
export default function ContextualQrPage() {
  return (
    <div className="bg-slate-950 text-white min-h-screen overflow-hidden">
      {/* HERO SECTION */}
      <section className="relative px-4 sm:px-6 lg:px-8 pt-10 pb-10 sm:pt-14 sm:pb-14 lg:pt-16 lg:pb-16 min-h-screen flex items-start">
        {/* Animated background */}
        <div className="pointer-events-none absolute inset-0 -z-10">
          ...
          <motion.div
            ...
            animate={{ scale: [1, 1.4, 1], opacity: [0.25, 0.5, 0.25], x: [0, -40, 0], y: [0, 50, 0] }}
          ...
          <motion.div
            ...
            animate={{ scale: [1, 1.2, 1], opacity: [0.25, 0.45, 0.25], x: [0, 30, 0], y: [0, -40, 0] }}
            ...
            >
              ...
              <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-black leading-tight mb-5 lg:mb-6">
                ...
                <motion.span
                  ...
                  animate={{ textShadow: [`0 0 20px ${logoGreen}40`, `0 0 40px ${logoGreen}80`, `0 0 20px ${logoGreen}40`] }}
                  transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
                >
                  adapts
                </motion.span>
              ...
              <p className="text-sm sm:text-base lg:text-lg text-slate-300 mb-6 lg:mb-8 max-w-2xl mx-auto lg:mx-0">
                We build smart QR solutions that respond to location, time, user type, or usage conditions — enabling businesses, schools, and cities to deliver relevant experiences and automate workflows.
                From creation to deployment, monitoring, and security, we support the full QR lifecycle with a practical, scalable approach.
              </p>

              <div className="flex flex-wrap gap-2 justify-center lg:justify-start text-xs sm:text-sm mb-8">
                {[
                  { icon: "🧠", label: "Centralized management", border: "border-emerald-400/40" },
                  { icon: "📊", label: "Live scan analytics", border: "border-sky-400/40" },
                  { icon: "🔒", label: "Enterprise security", border: "border-fuchsia-400/40" },
                ].map((badge, i) => (
                  <motion.span
                    key={i}
                    className={`inline-flex items-center gap-2 rounded-full border ${badge.border} px-4 py-2 bg-slate-950/60 backdrop-blur-md`}
      ...
      <section className="py-16 sm:py-20 lg:py-24 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-slate-50 to-white">
        <div className="mx-auto max-w-7xl">
          ...
          >
            <p className="text-sm lg:text-base font-semibold tracking-widest text-emerald-500 mb-3">WHAT WE PROVIDE</p>
            ...
            {features.map((feature, idx) => (
              ...
              >
                ...
                <div className="flex flex-col flex-1">
                  ...
                  <h3 className="text-base sm:text-lg lg:text-xl font-bold text-slate-900 mb-2">{feature.title}</h3>
                  <p className="text-xs sm:text-sm lg:text-base text-slate-600 leading-relaxed">{feature.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-16 sm:py-20 lg:py-28 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-slate-900 via-slate-950 to-slate-900 text-slate-50">
        <div className="max-w-7xl mx-auto relative">
          ...
          >
            <p className="text-xs sm:text-sm lg:text-base font-semibold tracking-widest text-emerald-400 mb-3">QR SOLUTION PROCESS</p>
                ...
                {stages.map((stage, idx) => (
                  ...
                  >
                    <div className="flex flex-col items-center text-center">
                      <motion.div
                        className={`flex items-center justify-center h-12 w-12 rounded-2xl border-2 bg-slate-950 shadow-lg text-lg font-bold ${idx === 0 ? "border-emerald-400 text-emerald-300" : idx === 1 ? "border-sky-400 text-sky-300" : idx === 2 ? "border-amber-400 text-amber-300" : idx === 3 ? "border-fuchsia-400 text-fuchsia-300" : "border-emerald-500 text-emerald-300"}`}
                        whileHover={{ scale: 1.1, rotate: 5 }}
                      >
                        {stage.icon}
                      </motion.div>
                      <p className="mt-4 text-xs font-semibold tracking-widest text-emerald-300 uppercase">Step {idx + 1}</p>
                      <h3 className="mt-2 text-sm font-bold text-slate-50 leading-tight px-2">{stage.title}</h3>
                      <p className="mt-2 text-xs text-slate-300 leading-relaxed max-w-[200px]">{stage.desc}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
              ...
              {stages.map((stage, idx) => (
                ...
                >
                  ...
                  <div className="flex gap-4 p-5 rounded-2xl bg-slate-900/80 border border-emerald-500/20">
                    <div className={`flex items-center justify-center h-12 w-12 shrink-0 rounded-xl border-2 bg-slate-950 shadow-lg text-lg font-bold ${idx === 0 ? "border-emerald-400 text-emerald-300" : idx === 1 ? "border-sky-400 text-sky-300" : idx === 2 ? "border-amber-400 text-amber-300" : idx === 3 ? "border-fuchsia-400 text-fuchsia-300" : "border-emerald-500 text-emerald-300"}`}>
                      {stage.icon}
                    </div>
                    <div className="flex-1">
                      <p className="text-xs font-semibold tracking-widest text-emerald-300 uppercase mb-1">Step {idx + 1}</p>
                      <h3 className="text-base font-bold text-slate-50 mb-2">{stage.title}</h3>
      ...
      <section className="px-4 sm:px-6 lg:px-8 py-16 sm:py-20 lg:py-24 bg-white">
        ...
        >
          ...
          >
            <p className="text-sm lg:text-base font-semibold tracking-widest text-emerald-500 mb-3">INDUSTRY APPLICATIONS</p>
            <h2 className="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-extrabold text-slate-900">Applications across sectors</h2>
            <p className="mt-4 text-sm sm:text-base text-slate-600 max-w-2xl mx-auto">Our platform serves diverse industries with tailored QR solutions.</p>
          </motion.div>

          <div className="grid gap-5 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {exampleApps.map((item, idx) => (
              <motion.div key={item.title} variants={itemVariants} whileHover={{ y: -8, scale: 1.04 }} className="relative group">
                <div className={`absolute -inset-[2px] rounded-2xl opacity-0 group-hover:opacity-100 blur-sm transition-opacity duration-300 ${idx % 3 === 0 ? "bg-gradient-to-br from-emerald-400 via-teal-400 to-sky-400" : idx % 3 === 1 ? "bg-gradient-to-br from-fuchsia-500 via-pink-500 to-orange-400" : "bg-gradient-to-br from-indigo-500 via-sky-500 to-cyan-400"}`} />
                <div className="relative rounded-2xl bg-white border border-slate-100 px-5 py-6 shadow-lg transition-shadow duration-300 group-hover:shadow-2xl">
                  <div className="mb-4 flex items-center justify-center">
                    <div className={`h-14 w-14 rounded-xl flex items-center justify-center text-2xl text-white ${idx % 3 === 0 ? "bg-gradient-to-br from-emerald-500 to-teal-500" : idx % 3 === 1 ? "bg-gradient-to-br from-fuchsia-500 to-orange-400" : "bg-gradient-to-br from-indigo-500 to-sky-500"}`}>
                      {item.icon}
                    </div>
                  </div>
                  <h4 className="font-bold mb-2 text-base lg:text-lg text-slate-900 text-center">{item.title}</h4>
                  <p className="text-xs sm:text-sm text-slate-600 text-center leading-relaxed">{item.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-16 sm:py-20 lg:py-24 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-slate-50 to-white">
        <div className="max-w-7xl mx-auto">
          <motion.div
            ...
            className="text-center mb-12 lg:mb-16"
          >
            <p className="text-sm lg:text-base font-semibold tracking-widest text-emerald-500 mb-3">OUR APPROACH</p>
            <h2 className="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-extrabold text-slate-900 max-w-3xl mx-auto">Building something extraordinary</h2>
          </motion.div>

          <div className="grid md:grid-cols-3 gap-6 lg:gap-8">
            {[
              { title: "Vision", desc: "To revolutionize how businesses and organizations leverage QR technology for seamless digital experiences.", icon: "🎯", color: "from-emerald-500 to-teal-500" },
              { title: "Mission", desc: "Creating an enterprise-grade QR platform that's powerful yet simple, serving everyone from startups to enterprises.", icon: "🚀", color: "from-sky-500 to-blue-500" },
              { title: "Values", desc: "Innovation, security, and user experience drive every decision we make as we build the future of QR solutions.", icon: "💎", color: "from-fuchsia-500 to-pink-500" },
            ].map((item, idx) => (
              <motion.div
                key={item.title}
                initial={{ opacity: 0, y: 30 }}
              ...
              >
                <div className={`inline-flex h-14 w-14 items-center justify-center rounded-xl bg-gradient-to-br ${item.color} text-2xl text-white mb-4`}>{item.icon}</div>
